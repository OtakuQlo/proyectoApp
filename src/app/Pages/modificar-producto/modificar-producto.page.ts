...
export class ModificarProductoPage implements OnInit {
  ...
  constructor(
    ...
    this.activatedRouter.queryParams.subscribe((param) => {
      if (this.router.getCurrentNavigation()?.extras.state) {
        this.idpublicacion =
          this.router.getCurrentNavigation()?.extras?.state?.['idpubliE'];
        this.modelo =
  ...
  irPaginaPublicaciones() {
    ...
    if (this.modelo.length < 1) {
      pass = 1;
      this.labelModelo =
        'El modelo debe contener como minimo 2 caracteres no especiales.';
    } else {
      this.labelModelo = '';
    }
    ...
    if (this.descrp == '' || this.descrp.length < 10) {
      pass = 1;
      this.labelDescrp =
        'La descripcion debe contener como minimo 10 caracteres.';
    } else {
      this.labelDescrp = '';
    }
    ...
    if (
      !this.regexnumber.test(this.kilometraje) ||
      this.kilometraje == '' ||
      parseInt(this.kilometraje) == 0
    ) {
      pass = 1;
      ...
      if (parseInt(this.kilometraje) > 320000) {
        ...
        this.labelKilometraje =
          'El auto no puede ser publicado si su kilometraje pasa los 320000.';
      }
    } else {
      this.labelKilometraje = '';
    }
    ...
    if (
      parseInt(this.anos) == 0 ||
      this.anos == '' ||
      !this.regexnumber.test(this.anos)
    ) {
      pass = 1;
    ...
    if (
      parseInt(this.precio) == 0 ||
      this.precio == '' ||
      !this.regexnumber.test(this.precio)
    ) {
      pass = 1;
  ...
  modificarPublicacion() {
    ...
    this.db.presentToast('bottom', 'Publicacion actualizada correctamente');
